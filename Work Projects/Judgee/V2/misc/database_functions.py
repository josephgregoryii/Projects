...
def insertFeedback(dictionary):
    ...
    try:
        mycursor = mydb.cursor()
        query = "INSERT INTO Feedback (user_id, trait_id, num_ratings, avg_rating) VALUES (%s, %s, %s, %s)"
        val = (dictionary['user_id'], dictionary['trait_id'], dictionary['num_ratings'], dictionary['avg_rating'])
        mycursor.execute(query, val)
...
def getFeedback(user_id):
    ...
    try:
        mycursor = mydb.cursor()
        query = "SELECT t.trait_name, f.num_ratings, f.avg_rating FROM Feedback f JOIN Traits t USING (trait_id) WHERE f.user_id = %d" % user_id 
        mycursor.execute(query)
        result = mycursor.fetchall()
...
def chooseRandomUserAndTrait(current_user):
    # create connection
    mydb = connectDatabase()
