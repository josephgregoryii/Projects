...
def credits():
    ...
    if request.method == "POST" and request.form['purchase-button']:
        
        while True:
            #result is an available trait of the current user
            result = db.chooseRandomUserAndTrait(user_id, False)

            #from_db contains only the feedback available for purchase
            from_db = db.getFeedback(user_id, True)
            count = db.countFeedback(user_id)
            if (len(from_db) == 0):
                display_text = "No feedback available for purchase"
                break
            if len(from_db) == 7:
                display_text = "You have unlocked all your feedback!"
                break

            try:
                new_trait = result["trait_name"]
                trait_id = result["trait_id"]
                num_ratings = from_db[new_trait][0]
                avg_rating = from_db[new_trait][1]
                ...
                display_text = "Congratulations! You have purchased the feedback of %s" % new_trait.capitalize()

                #update feedback to show that it has been purchased
                db.updateFeedback(to_db, True)

                #update amount of credits
                db.updateCredit(user_id, credits-1)
                credits = db.getCredit(user_id) 

                break
            except KeyError as e:
                app.logger.info("attempting to find new trait")
           
        return render_template("credits.html", 
                                full_name = user.name, 
                                credits = credits, 
                                user_img = img_to_front, 
                                new_trait = new_trait, 
...
#------------------------No Feedback Available--------------------------------#
    ...
    if len(from_db) == 0:
        ...
        to_front = [db_front]
        return render_template('my-feedback.html', 
                                items = to_front, 
                                bar = to_front_bar, 
                                full_name = user.name, 
                                user_img = img_to_front)
...
#------------------------Feedback Available-----------------------------------#
        ...
        for key in from_db:
            ...
            to_front_table = {
                            ...
                            'average_score'  : round(from_db[key][1],2)
                            }
...
def rateOthers():
    ...
    if request.method == 'POST':
        ...
        return render_template("rate-others.html", 
                                trait = trait, 
                                img = img_to_front, 
                                full_name = user.name, 
                                user_img = user_img_to_front)
